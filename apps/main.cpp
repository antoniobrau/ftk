...
void tracks_simulation(const std::string& filename) {
    ...
    DataHandler dataHandler(filename, 100000, black_list, 1,64, 80);
    // DataHandler dataHandler(filename, 100, 1);

    ThreadPool pool(POOL_SIZE);
    // FTK ftk(100000000, dataHandler, pool, 0.999, 5000);
    FTK ftk(20000000, dataHandler, pool, 0.75, 10000);
    ...
    saveOccorrenzeToCSV(ftk, out_file, ftk.get_total_iterations());
}
